                ...
                <table class="table table-striped ">
                    ...
                    @foreach($reports as $report)
                        @if($report -> type_report=='مهرب')
                                <tr class="reportRow">
                                    <td><a class="nav-link "   href="{{url('operationsManagement/detailsSmuggledReport',$report -> report_no)}}">{{$report -> app_user_name}}</a></td>
                                    <td><a class="nav-link  "  href="{{url('operationsManagement/detailsSmuggledReport',$report -> report_no)}}">{{$report -> report_date}} </a></td>
                                    <td ><a class="nav-link  " href="{{url('operationsManagement/detailsSmuggledReport',$report -> report_no)}}">{{$report -> type_report}}</a></td>
                                    <td class="align-middle white-space-nowrap">
                                        <div class="dropdown font-sans-serif">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right border py-2" aria-labelledby="dropdown0">
                                                <a class="dropdown-item" href="{{url('operationsManagement/detailsSmuggledReport',$report -> report_no)}}">عرض</a>
                        ...
                        @else
                            <tr class="reportRow">
                                    <td><a class="nav-link "   href="{{url('operationsManagement/detailsReport',$report -> report_no)}}">{{$report -> app_user_name}}</a></td>
